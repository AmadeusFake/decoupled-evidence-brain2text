...
- 评分逻辑保持与训练一致（候选端 L2，逐时序不池化）
- 先加载候选池（去重），建立“句子别名→句索引”
- 使用 run_dir/records/subject_mapping.json 做 subject 匹配（鲁棒归一化）
- 可选从 ckpt 读取 exp(logit_scale)，保证温度与训练一致
- Window-Vote：Top-K 内按句聚合后，回填到该句所有候选窗口（预建句桶以降复杂度）
- 可选 QCCP（默认关）

用法（示例）：
python -u train/retrieval_window_vote.py \
  --test_manifest  <.../fold0/test.jsonl> \
  --run_dir        <.../run_baseline/none_baseline_SLR_fold0_...> \
  --use_best_ckpt \
  --device cuda --amp bf16 \
  --chunk_size 256 --topk_window 1024 --gamma 0.7 --gate --q_quantile 0.9 \
  --sent_agg logsumexp --topk 1,5,10 --save_topk 10
"""
...
def evaluate(args):
        ...
        for j_in_group, global_j in enumerate(q_indices):
            ...
            if save_topk_k > 0:
                ...
                rec = {
                    ...
                    "window_vote": {"topk": int(args.topk_window), "gamma": float(args.gamma),
                                    "q": float(args.q_quantile), "gate": bool(args.gate), "agg": args.sent_agg},
    ...
    if args.use_qccp:
        ...
        for g_idx in tqdm(range(len(groups)), desc="Evaluate (QCCP + WindowVote)"):
            process_query_indices(groups[g_idx])
    else:
        B = len(test_rows)
        cs = int(args.chunk_size)
        for st in tqdm(range(0, B, cs), desc="Evaluate (WindowVote)"):
            ed = min(B, st + cs)
    ...
    log("==== Retrieval (Window-Vote{} ) Results ====".format(" + QCCP" if args.use_qccp else ""))
...
def parse_args():
    ...
    p.add_argument("--q_quantile", type=float, default=0.9)
    p.add_argument("--sent_agg", type=str, default="logsumexp", choices=["logsumexp", "mean", "max"])
