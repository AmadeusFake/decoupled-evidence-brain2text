...
# -*- coding: utf-8 -*-
"""
EEG Training script (Compatible with Brain Magick / SimpleConv)
Fixed: 
1. PaperClipLoss using Fixed Temperature (Scale ~ 14.3)
2. Removed Matplotlib to prevent crashes/hangs
3. Added rank_zero checks for logging safety
4. Added numerical stability (eps, clamp)
"""
...
class EEGDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        if self.normalize:
            ...
            # Safe normalization: ensure std is at least a small epsilon
...
class PaperClipLoss(nn.Module):
    def __init__(self, target_T: int = TARGET_T, pool: bool = False,
        ...
        # [FIX] Logic for Fixed vs Learnable Temperature
        # init_temp = 0.07 corresponds to scale ~14.3
    ...
    def forward(self, meg_f: torch.Tensor, aud_f: torch.Tensor):
        ...
        # 4. Normalize (with eps for safety)
        m_norm = F.normalize(m_flat, p=2, dim=1, eps=1e-6)
        a_norm = F.normalize(a_flat, p=2, dim=1, eps=1e-6)

        # 5. Compute Logit Scale (Clamped for safety)
        # Even if fixed, clamp prevents numerical issues if manually tampered
...
def batch_retrieval_metrics(logits: torch.Tensor, ks=(1, 5, 10)) -> Dict[str, float]:
    ...
    ks = tuple(int(k) for k in ks if k >= 1 and k <= B)
...
class MetricLogger:
    ...
    def write_step(self, phase: str, step: int, epoch: int, loss: float, lr: float | None,
        ...
        if metrics: rec.update({k: float(v) for k, v in metrics.items()})
        try:
    ...
    def write_epoch(self, epoch: int, train_loss: float | None, val_loss: float | None,
        ...
        if train_metrics: rec.update({f"train_{k}": float(v) for k, v in train_metrics.items()})
        if val_metrics:   rec.update({f"val_{k}": float(v) for k, v in val_metrics.items()})
        try:
    ...
    def export_tables(self):
        # [FIX] Only export CSV. No plotting to prevent hanging.
...
class EEGLitModule(pl.LightningModule):
    def __init__(self, model_cfg: dict, lr: float = 3e-4, weight_decay: float = 0.0,
        ...
        # [FIX] Force fixed temperature logic based on args
    ...
    def training_step(self, batch, batch_idx):
        ...
        # [FIX] Rank zero check for logger
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # [FIX] Rank zero check
        if self.trainer.is_global_zero and self.metric_logger is not None:
             # Only log sample steps occasionally to save IO
    ...
    def on_validation_epoch_end(self):
        ...
        # [FIX] Rank zero check for writing epoch logs & console print
        if self.trainer.is_global_zero:
            ...
            logger.info(f"[Epoch {int(self.current_epoch):03d}] train_loss={self._fmt4(tr)} | val_loss={self._fmt4(va)} | best_val={self._fmt4(self._best_val)}")
            logger.info(f"Train: Top1={_fmt_pct(mget(trm,'top1'))}  Top5={_fmt_pct(mget(trm,'top5'))}  Top10={_fmt_pct(mget(trm,'top10'))}  MRR={self._fmt4(mget(trm,'mrr'))}  MeanRank={self._fmt4(mget(trm,'mean_rank'))}")
            logger.info(f"Valid: Top1={_fmt_pct(mget(vam,'top1'))}  Top5={_fmt_pct(mget(vam,'top5'))}  Top10={_fmt_pct(mget(vam,'top10'))}  MRR={self._fmt4(mget(vam,'mrr'))}  MeanRank={self._fmt4(mget(vam,'mean_rank'))}")
    ...
    def configure_optimizers(self):
        ...
        def lr_lambda(step: int):
            ...
            if step < warmup: return float(step + 1) / float(max(1, warmup))
...
def save_records(run_dir: Path, cfg: dict, best_ckpt_path: str | None, subject_map_path: Optional[str] = None, registry: Optional[SubjectRegistry] = None):
    ...
    with open(rec_dir / "config.json", "w", encoding="utf-8") as f: json.dump(cfg, f, indent=2, ensure_ascii=False)
    if best_ckpt_path:
        with open(rec_dir / "best_checkpoint.txt", "w", encoding="utf-8") as f: f.write(best_ckpt_path + "\n")
    if subject_map_path:
        with open(rec_dir / "subject_mapping_path.txt", "w", encoding="utf-8") as f: f.write(subject_map_path + "\n")
...
def parse_args():
    ...
    # --- Brain Magick / SimpleConv Parameters (Explicit) ---
    p.add_argument("--spatial_channels", type=int, default=270, help="merger_channels")
    p.add_argument("--d_model", type=int, default=320, help="hidden.meg")
    
    p.add_argument("--backbone_depth", type=int, default=10, help="simpleconv.depth")
    p.add_argument("--backbone_kernel", type=int, default=3, help="simpleconv.kernel_size")
    p.add_argument("--dilation_period", type=int, default=5, help="simpleconv.dilation_period")
    p.add_argument("--glu_mult", type=int, default=2, help="simpleconv.glu")
    p.add_argument("--dropout", type=float, default=0.0, help="simpleconv.dropout")
    
    p.add_argument("--use_subjects", action="store_true", help="simpleconv.subject_layers")
    p.add_argument("--subject_layer_pos", type=str, default="early", choices=["early","late","none"])
    p.add_argument("--spatial_dropout_p", type=float, default=0.2, help="merger_dropout")
...
def main():
    ...
    lit = EEGLitModule(model_cfg=model_cfg, lr=scaled_lr, weight_decay=args.weight_decay, warmup_ratio=args.warmup_ratio,
                       max_epochs=args.max_epochs, optimizer_name=args.optimizer, metric_logger=metric_logger,
                       loss_use_temp=args.loss_temp, metrics_every_n_steps=args.metrics_every_n_steps)

    cfg_to_save = {"args": vars(args), "model_cfg": model_cfg, "effective_bsz": effective_bsz,
                   "scaled_lr": scaled_lr, "start_time": datetime.now(timezone.utc).isoformat(),
    ...
    ckpt_cb = ModelCheckpoint(dirpath=run_dir / "checkpoints",
    ...
    trainer = pl.Trainer(max_epochs=args.max_epochs,
                         ...
                         devices=devices, precision=precision, default_root_dir=str(run_dir),
                         accumulate_grad_batches=args.accumulate_grad_batches,
                         callbacks=[ckpt_cb, es_cb], log_every_n_steps=50,
                         deterministic=False, gradient_clip_val=args.gradient_clip_val)
