...
---------------------------------------------------------------
- One-way contrastive training (brain -> audio); in-batch negatives on the same GPU.
- Linearly interpolate BOTH modalities to T=360; **no temporal pooling** (keep time dimension).
- Scoring rule: **only candidates (audio side) are L2-normalized over (C,T)**; estimates (MEG side) are NOT normalized.
...
Other engineering improvements (do not change training logic)
-------------------------------------------------------------
1) SubjectRegistry: stable subject indexing (supports multi-dataset namespaces).
2) Cleaner logs and retrieval metrics (Top-1/5/10, MRR, MeanRank).
...
class SubjectRegistry:
    """
    Stable "subject -> index" registry with optional multi-dataset namespaces.

    - Composite key: "<namespace>:<subject_id>" (or plain subject_id if namespace is empty)
    - Build: collect the union of subject_id across manifests (with namespaces), sort, then assign stable indices
    - Persistence: save/load to JSON so that training and downstream evaluation use exactly the same mapping
    """
    ...
    def index_of(self, ns: str, sid: str, fallback_to_first: bool = True) -> int:
        ...
        # Unknown subject: do not expand model tables; optionally fall back to the first index (impact is negligible).
...
def _ensure_CxT(x: np.ndarray) -> np.ndarray:
    """Ensure 2D arrays are in channel×time layout. Heuristic: MEG usually C≈270, T≈360; if rows<=cols treat as [C,T]."""
    if x.ndim != 2:
...
class MEGDataset(Dataset):
    """
    Read a jsonl manifest (Stage-3 fully_preprocessed):
    - meg_win_path: robust-normalized + clamped [C,T] float32
    - audio_feature_path: [1024,T] float32 (Stage-2 is typically interpolated to T=360; if not, loss will handle interpolation)
    - sensor_coordinates_path: [C,3] float32 with x,y in [0,1], z=0
    - subject_id: subject identifier (combined with namespace to form a stable SubjectRegistry key)
    """
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        meg = _ensure_CxT(meg)
        if self.normalize:
            # Ablation-only; the paper path does NOT re-standardize (Stage-3 already robust-normalized + clamped).
        ...
        item = {
            ...
            "audio_feature": torch.from_numpy(aud),   # [T,1024] or [1024,T]; handled in loss
...
class MEGDataModule(pl.LightningDataModule):
    ...
    def _collate(self, batch: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        aud = torch.stack(col["audio_feature"], dim=0) # [B,?,?] (loss will convert to [B,D,T])
...
class PaperClipLoss(nn.Module):
    """
    Paper/reference-aligned contrastive loss (one-way: brain -> audio)
    - In-batch negatives only (on the same GPU).
    - Linear interpolation to T=360 before scoring.
    - **Only candidates (audio) are L2-normalized** over (C,T); estimates (MEG) are NOT normalized.
    - **No temporal pooling** (keep time dimension); optional: center/trim (off by default).
    - No temperature by default; enable learnable temperature with --loss_temp.
    """
    def __init__(
        self,
...
class MetricLogger:
    ...
    def export_tables_and_plots(self):
        """Aggregate JSONL into CSV/XLSX, and save PNG curves (loss, lr, Top-1)."""
        import csv
        # CSV
        ...
        # XLSX (optional dependency; auto-skip if missing)
        try:
            import pandas as pd
...
class MEGLitModule(pl.LightningModule):
    def __init__(self, model_cfg: dict, lr: float = 3e-4, weight_decay: float = 0.0,
        ...
        # —— Compatibility between old/new encoders:
        #    - New versions do not expose out_timesteps
        #    - If present, set out_timesteps=None to enforce "no temporal pooling" during training
        enc_sig = inspect.signature(UltimateMEGEncoder).parameters
        enc_cfg = dict(model_cfg)
        if "out_timesteps" in enc_sig:
            enc_cfg["out_timesteps"] = None  # paper-aligned: no temporal pooling during training
        self.model = UltimateMEGEncoder(**enc_cfg)

        # Paper-aligned defaults: one-way; candidate-only L2 normalization; no temperature; no pooling
        if loss_use_l2:
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Per-batch retrieval metrics (accumulate every step; write step logs at a chosen frequency)
    ...
    def on_validation_epoch_end(self):
        ...
        # Console summary table (aligned formatting)
...
def parse_args():
    ...
    # ================= Model (aligned with BrainMagick simpleconv) =================
    ...
    # SimpleConv backbone params (aligned with config.yaml.simpleconv)
    ...
    # Legacy args kept for compatibility (mostly unused by the new encoder)
    ...
    # Placeholder arg kept for CLI compatibility (unused)
...
def main():
    ...
    # Suggest enabling TF32 / high matmul precision on A100 for better throughput.
    try:
    ...
    # Infer n_subjects: default to registry size (union across train/val/test)
    ...
    model_cfg = dict(
        ...
        # Compatibility only; ignored in the new encoder
    ...
    lit = MEGLitModule(
        ...
        loss_use_l2=args.loss_use_l2 if hasattr(args, "loss_use_l2") else False,  # compatibility
    ...
    try:
        ...
        # Export tables/plots even if training is interrupted.
