...
# -*- coding: utf-8 -*-
"""
Stage 2: Audio feature extraction with layer flags (FAST + RESUMABLE).

- Model: wav2vec2 (default: facebook/wav2vec2-large-xlsr-53)
- Layers: --w2v_layers (e.g. last4 / 14-18 / 14,15,16,17,18)
- Aggregation: --w2v_agg {mean|concat}
- Time axis: interpolated to T=360 (3 s @ 120 Hz)
- Resume: skip existing .npy files with verified shape
"""
...
def content_id_for_sample(audio_path: Path, onset_s: float, offset_s: float) -> str:
    """Stable content-level ID for naming."""
...
def build_audio_index(roots: List[Path]) -> Dict[str, Path]:
    """Case-insensitive index: absolute, relative, and basename."""
    index: Dict[str, Path] = {}
    for root in roots:
        if not root.exists():
            continue
...
def guess_audio_roots(samples: List[Dict]) -> List[Path]:
    """Heuristic audio root discovery."""
    roots = set()
    for s in samples:
...
def build_layer_indices(spec: str, hs_len: int) -> List[int]:
    """Parse layer spec into hidden_state indices."""
    s = spec.strip().lower()
    m = re.match(r"last-?(\d+)$", s)
    if m:
        k = int(m.group(1))
        if k <= 0 or k > hs_len:
            raise ValueError(f"Invalid last{k} for hs_len={hs_len}")
...
class AudioWinDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        try:
            ...
            logging.error(f"torchaudio.load failed: {rp} ({e}); zero-filled.")
            meta["_missing"] = True
            return {"wave": torch.zeros(desired), "meta": meta}
...
def wav2vec_forward(model, waves, device, use_bf16, layer_spec, agg):
    """Forward + layer selection + temporal interpolation."""
...
def load_done_set(out_manifest_dir, split, verify_existing, expected_shape):
    """Return completed {window_id: feature_path}."""
...
def main():
    ...
    audio_index = build_audio_index(guess_audio_roots(all_samples))
    ...
    for split, rows in splits.items():
        ...
        to_process = [r for r in rows if args.recompute_existing or r["window_id"] not in done]
        logger.info(f"[{split}] process={len(to_process):,} | skip={len(done):,}")
            ...
            for batch in tqdm(dl):
                ...
                for i, m in enumerate(batch["metas"]):
                    ...
                    cid = content_id_for_sample(
                        rp,
                        float(m["local_window_onset_in_audio_s"]),
                        float(m["local_window_offset_in_audio_s"]),
                    )
                    outp = feat_dir / f"{safe_name_from_id(cid)}.npy"

                    if not args.recompute_existing and verify_npy_shape(outp, expected_shape):
