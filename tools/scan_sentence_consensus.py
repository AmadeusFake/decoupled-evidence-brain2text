...
Goal
- Offline evaluate which sentence-level "consensus" formula works best, WITHOUT changing the model or re-ranking code.
- Given a group of queries (multiple windows from the same sentence), for each query we take top-K logits and apply a
  quantile threshold q; then aggregate into per-sentence bucket scores for that query. Finally, aggregate across queries
  (sum / mean / vote, etc.) to obtain group-level support over sentences and check whether Top-1 hits the GT sentence.

Outputs
- <run_dir>/results/sent_consensus_scan/scan_summary.json
- <run_dir>/results/sent_consensus_scan/scan_summary.csv
- Print Top-N best configs to stdout

Swept dimensions
- Window → sentence aggregation (within a single query): max / mean(top-m) / logsumexp(top-m) / soft-avg(T)
- Normalization: by kept_count / bucket_count / bucket_sqrt / none
- Across-queries aggregation: sum / mean / vote (each query votes its per-query top-1 sentence)
- Evidence knobs: topK ∈ {64,128,256,...}, q ∈ {0.95,0.98,...}, top-m ∈ {1,3,5}, soft T ∈ {1.5,2.5}

Notes
- Depends on models.meg_encoder.UltimateMEGEncoder (same as your current project)
- Same flavor as your eval/probe scripts: supports --use_best_ckpt / --use_ckpt_logit_scale / --amp bf16, etc.
"""
...
# ------------------------- Constants -------------------------
TARGET_T = 360       # target time length T
AUDIO_D = 1024
EPS = 1e-8
...
def load_model_from_ckpt(ckpt_path: Path, run_dir: Path, device: str) -> Tuple[UltimateMEGEncoder, Dict[str,Any]]:
    ...
    if "out_timesteps" in UltimateMEGEncoder.__init__.__code__.co_varnames:
        model_cfg["out_timesteps"] = None  # evaluation does not time-pool
...
# ------------------------- Consensus formula (window→sentence; sentence→group) -------------------------
...
def per_query_sentence_scores(
    ...
    *, topk:int, q:float,
    ...
    """
    Returns: sid -> score (this query's support for each sentence)
    """
    O = s_logits_o.size(0)
    K = min(int(topk), O)
...
def across_queries_aggregate(
    ...
    if mode in ("sum","mean"):
        ...
        for d in per_query_scores:
            if not d: continue
            # Each query votes for its per-query top-1 sentence
...
# ------------------------- Main flow (scan or single config) -------------------------
def evaluate_groups_logits(
    ...
    # Precompute GT window index in pool for all test rows
    ...
    for g in tqdm(groups, desc="Encode+Logits (Grouped)"):
        ...
        # Use the first query in the group to define GT sentence id
        g0 = gt_index[g[0]]
...
def run_single_or_scan(args):
    ...
    # Rough estimate of window-level R@1 as an "independence" upper bound p
    # NOTE: This is window-retrieval against window-GT R@1 (not identical to your main eval, but sufficient for estimating).
    ...
    for (topk,q,agg,topm,norm,across,soft_T) in tqdm(combos, desc="Scan combos"):
        ...
        for g_idx, (g, logits, gt_sid) in enumerate(zip(groups, logits_per_group, gt_sid_per_group)):
            ...
            if not s_scores:
                # No sentence score for this group (counted as failure)
                pass
            else:
...
def parse_args():
    ...
    p.add_argument("--sample_groups", type=int, default=0, help="only scan the first N groups; 0=all")

    p.add_argument("--mode", type=str, default="scan", choices=["scan","single"])

    # Scan lists (comma-separated strings)
