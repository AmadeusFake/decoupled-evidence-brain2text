...
Fast consistency checker for sensor coordinates.

Design:
- Group by (subject_id, session_id); load each coordinate file once (mmap).
- Verify that all samples within a session share the same sensor_coordinates_path.
- Spot-check a small number of MEG windows (default: 2) for channel-count consistency
  using mmap, avoiding per-row full I/O.
...
def group_by_session(rows):
    """Group manifest rows by (subject_id, session_id)."""
...
def _check_one_session(args):
    ...
    # 3) Spot-check MEG window channel counts
    checked = 0
...
def main():
    ...
    ap.add_argument("--manifest_dir", required=True, type=str)
    ap.add_argument("--report_csv", default="", type=str)
